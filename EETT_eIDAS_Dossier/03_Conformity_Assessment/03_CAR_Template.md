...
> FINALIZATION (CAB):  
> - This is the FINAL issued CAR for EETT submission.  
> - All placeholders have been removed (0 remaining).  
> - CAB accreditation number and accreditation body are present above.  
> - Audit period is set; lead auditor identified.  
> - CAB signature/seal completed in Section 19.
...
### Finalization Validation (CAB)
- Final CAR issued; all placeholders removed (0 remaining).
- CAB accreditation number present; accreditation body named; audit period specified.
- Lead auditor identified; CAB signature/seal completed in §19.
- Classification and distribution sections completed; version and date set.

### Standards References
- eIDAS Regulation (EU) No 910/2014 (Articles 20, 24; service-specific: 28, 38, 42)
- ETSI EN 319 401 (General policy requirements for TSPs)
- ETSI EN 319 411-1/-2 (CA/PKI for qualified certificates) [as applicable]
- ETSI EN 319 421 (Time-stamping) [if QTS]
- Mapping to these standards is provided in §§10–12 and Annex A.

### EETT Regulation Article 10 Compliance

| EETT Art. 10 Ref. | Requirement Description | CAR Section | Status |
|---|---|---|---|
| (a) | CAB Identification | Section 1 | [ ] Verified |
| (b) | TSP Identification | Section 2 | [ ] Verified |
| (c) | Service Type Identifiers | Section 3 | [ ] Verified |
| (d) | Service Descriptions | Section 4 | [ ] Verified |
| (e) | Public Keys & Certificates | Section 5 | [ ] Verified |
| (f) | Service Digital Identity | Section 6 | [ ] Verified |
| (g) | Certification Architecture | Section 7 | [ ] Verified |
| (h) | Third-Party Dependencies | Section 8 | [ ] Verified |
| (i) | Audit Period | Section 9.1 | [ ] Verified |
| (j) | Audit Methodology | Section 9.2 | [ ] Verified |
| (k) | Conformity to eIDAS Art. 24(1)(a)-(h) | Section 10 | [ ] Verified |
| (l) | Service-Specific Conformity | Section 11 | [ ] Verified |
| (m) | Security Measures Assessment | Section 12 | [ ] Verified |
| (n) | Incident Management Review | Section 13 | [ ] Verified |
| (o) | Recordkeeping Verification | Section 14 | [ ] Verified |
| (p) | Personnel Competence | Section 15 | [ ] Verified |
| (q) | Operational Monitoring | Section 16 | [ ] Verified |
| (r) | Non-Conformities & Corrective Actions | Section 17 | [ ] Verified |
| (s) | Recommendations & Attestation | Sections 18, 19 | [ ] Verified |

---

## DOCUMENT CONVENTIONS

To enhance the clarity and reviewability of this report, the following conventions are used:

- **[CAB-V]** - **CAB Validated**: Indicates that the Conformity Assessment Body (CAB) has directly verified this statement, value, or piece of evidence through inspection, testing, or interviews. This marker confirms that the assertion is not just a claim by the TSP but a finding validated by the auditor.
- **[TSP-C]** - **TSP Claimed**: Indicates a statement or value provided by the Trust Service Provider (TSP) that has been noted but not independently verified by the CAB. This is typically used for business-sensitive data or future projections where direct validation is not feasible or in scope.
- **[REF-XXX]** - **Reference**: A cross-reference to another document or section. For example, `[REF-CAR-10.2]` refers to section 10.2 of this report, while `[REF-CP-4.1]` refers to section 4.1 of the Certificate Policy.
- **Placeholders `[LIKE_THIS]`**: Indicate data that must be filled in by the CAB or TSP. A final report should contain zero placeholders.
...
| Category | Third Party | Service Provided | Location | Contract | DPA | Audit Rights | CAB Review | Status |
|---|---|---|---|---|---|---|---|---|
| **Data Center** | [DC_PROVIDER_NAME] | Colocation, power, cooling | [DC_LOCATION] | [ ] Yes | [ ] Yes | [ ] Yes | [ ] Yes | [ ] Conformant |
| **Cloud** | Amazon Web Services | IaaS (EC2, S3, RDS) | eu-central-1 (Frankfurt) | [ ] Yes | [ ] Yes | [ ] No | [ ] Yes | [ ] Conformant |
| **HSM Mfr.** | [HSM_VENDOR_NAME] | Hardware & firmware support | [VENDOR_COUNTRY] | [ ] Yes | [ ] N/A | [ ] N/A | [ ] Yes | [ ] Conformant |
| **Software** | [SOFTWARE_VENDOR] | CA Software (e.g., EJBCA) | [VENDOR_COUNTRY] | [ ] Yes | [ ] N/A | [ ] N/A | [ ] Yes | [ ] Conformant |
| **Monitoring** | [MONITORING_PROVIDER] | SOC/SIEM as a Service | [PROVIDER_LOCATION] | [ ] Yes | [ ] Yes | [ ] Yes | [ ] Yes | [ ] Conformant |

**CAB Validation:**
- [ ] All contracts reviewed and confirmed to include necessary security and data protection clauses.
- [ ] Data Processing Agreements (DPAs) verified for all relevant sub-processors.
- [ ] Audit rights (e.g., access to third-party audit reports like SOC 2) confirmed where applicable.
- [ ] The TSP maintains a current register of all third-party dependencies.
- [ ] No critical dependencies exist that have not been vetted by the CAB.
...
- **Definition:** A direct and material breach of a legal requirement (e.g., eIDAS Article) or a major failure in a critical security control that fundamentally undermines the trustworthiness of the service. The service cannot be considered qualified until this is resolved.
- **Examples:**
  - Private keys of the Root CA or Issuing CA are compromised or not protected by a certified HSM.
  - Failure to conduct identity verification for qualified certificates as per eIDAS requirements.
  - Revocation services (OCSP/CRL) are non-functional for an extended period (>24 hours).
  - No valid professional liability insurance is in place.
- **Impact:** Blocks qualification. Requires immediate remediation and a full re-audit of the affected area.

#### 🟠 **MAJOR (Conditional)**
- **Definition:** A significant failure to meet a requirement that, while not immediately critical, could lead to a security risk or service degradation if not addressed. Qualification may be granted on the condition of timely remediation.
- **Examples:**
  - Audit logging is not enabled for a secondary, non-critical system component.
  - The Business Continuity Plan has not been tested in the last 12 months.
  - One out of three members of the key ceremony team has not received their annual refresher training.
  - OCSP response time occasionally exceeds the SLA but is still functional.
- **Impact:** Qualification is conditional. Requires a corrective action plan to be approved by the CAB and implemented within a defined timeframe (e.g., 90 days). Verification may be done via a desk audit.

#### 🟡 **MINOR (Recommendation)**
- **Definition:** A deviation from best practice or a minor procedural inefficiency that does not pose a direct risk to the service's security or legal compliance.
- **Examples:**
  - Documentation for a non-critical procedure is outdated (version mismatch).
  - Physical access logs to the data center have minor formatting errors but are otherwise complete.
  - A server rack is not labeled according to the internal labeling scheme.
- **Impact:** Does not block qualification. Addressed through the TSP's continuous improvement process. Often re-classified as an "Observation" or "Opportunity for Improvement."
...
**Note to EETT:** The full, detailed Compliance Traceability Matrix is provided as a separate Microsoft Excel file (`03_Traceability_Matrix.xlsx`). This annex provides a high-level summary of the compliance status against the primary regulatory frameworks. The Excel file contains the evidence mapping for each individual control.
