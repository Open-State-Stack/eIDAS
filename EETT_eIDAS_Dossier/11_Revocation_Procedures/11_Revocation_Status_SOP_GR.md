...
-   **Προσωπικό:** Τουλάχιστον 2 **γεωγραφικά κατανεμημένοι** χειριστές ανά βάρδια (1 στο κύριο NOC, 1 απομακρυσμένα από το SOC) για την εξάλειψη μεμονωμένων σημείων αποτυχίας (single point of failure).
-   **Γλώσσες:** Ελληνικά, Αγγλικά.
-   **Διαδικασία & Script Χειριστή:**
    -   Κάθε χειριστής ακολουθεί υποχρεωτικά το έγγραφο `DOC-REVOKE-S01_Revocation_Hotline_Script.md` (βλ. 11_Revocation_Procedures/), το οποίο περιλαμβάνει βήματα για την αποφυγή social engineering και τη διαχείριση κλήσεων υπό πίεση.
    -   **Βήμα 1: Ταυτοποίηση.** Ο χειριστής ζητά το ονοματεπώνυμο και τον σειριακό αριθμό του πιστοποιητικού.
    -   **Βήμα 2: Έλεγχος Γνησιότητας.** Ο χειριστής θέτει 3 ερωτήσεις ασφαλείας από τη λίστα που ορίστηκε κατά την εγγραφή.
    -   **Βήμα 3: Απόφαση.**
        -   **Επιτυχής Έλεγχος:** Το αίτημα θεωρείται έγκυρο και προχωρά άμεσα στην ανάκληση.
        -   **Διαδικασία Κλιμάκωσης Έκτακτης Ανάγκης (Emergency Escalation Path):** Εάν ο καλών αποτύχει στον έλεγχο αλλά ισχυρίζεται ενεργή διαρροή κλειδιού, ο χειριστής **ΔΕΝ** απορρίπτει το αίτημα. Αντ' αυτού, κλιμακώνει **ΑΜΕΣΩΣ** το περιστατικό στον εφημερεύοντα **Incident Manager (CISO/SOC Lead)**. Ο Incident Manager έχει την εξουσία να λάβει απόφαση βασισμένη σε ρίσκο, συμπεριλαμβανομένης της προσωρινής αναστολής του πιστοποιητικού (`certificateHold`) εν αναμονή περαιτέρω ελέγχου.
