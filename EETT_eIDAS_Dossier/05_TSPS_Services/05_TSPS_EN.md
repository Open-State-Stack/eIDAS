...
<!-- **Guidance:** This is the "how-to" manual. While the TSP Policy (Doc 04) makes high-level promises, this TSPS provides the detailed, auditable procedures that prove *how* those promises are met. Every statement here must accurately reflect your day-to-day operations. -->
## 1. INTRODUCTION

### 1.1. Purpose
<!-- Explain that this document provides the operational detail behind the high-level TSP Policy. -->
...
### 1.2. Scope
<!-- Clearly state which specific services this TSPS covers. This must align with the services declared in your other application documents. -->
...
### 1.3. Compliance
<!-- List the primary regulations and standards this document adheres to. This demonstrates to the auditor that you understand the regulatory landscape. -->
...
<!-- **Guidance:** This section details your public commitments. The repository is your public library of policies and certificates, while the revocation information (OCSP/CRL) is a critical, real-time service for relying parties. -->
## 2. PUBLICATION AND REPOSITORY PRACTICES
...
<!-- **Guidance:** This is one of the most heavily audited sections. Your procedures for identifying individuals and legal entities must be robust and strictly follow the methods permitted by eIDAS and EETT. Describe the exact steps your Registration Authority (RA) operators will follow. -->
## 3. IDENTIFICATION AND AUTHENTICATION PROCEDURES
...
<!-- **Guidance:** Describe the technical and procedural steps for handling certificate requests after a subscriber's identity has been verified. -->
### 3.3. RA Vetting Levels (ETSI EN 319 411-2, eIDAS Art. 24)

The Registration Authority (RA) operates under documented procedures and separation of duties to ensure identity vetting meets the rigor required for qualified certificates.

- Natural persons (QES):
  - In-person RA office: original national ID/passport verification, live presence check, liveness/face match as applicable. Evidence retained per §11.
  - Remote video identification: real-time session with trained RA operator, anti-spoofing checks, document security features, liveness, facial similarity, evidence capture and retention. Aligned with ETSI EN 319 411-2 and national guidance.
  - eID/eIDAS-notified means: acceptance of notified electronic identification means at substantial/high LoA, with binding to certificate subject attributes.

- Legal persons (QSeal):
  - Legal entity verification: authoritative register (e.g., GEMI) extract, organizationIdentifier (VAT/GEMI) binding, proof of representation (board resolution/POA).
  - Representative’s identity: same rigor as natural person vetting; binding to legal entity.

- Approval & controls:
  - Four-eyes principle for approvals; issuer cannot approve own requests; automated conflict-of-interest checks.
  - High-risk flags (name collisions, PEP/sanctions, cross-border anomalies) trigger senior RA review and enhanced due diligence.
  - Evidence retention: all vetting artifacts (video, screenshots, register extracts) retained per §11 and privacy controls in §3 and §11.

## 4. CERTIFICATE APPLICATION, RENEWAL, AND RE-KEY
### 4.0 Certificate Profiles and CP/CPS Cross-References
-   Certificate profiles (OIDs, key usages, extended key usages, validity periods, signature algorithms) for each qualified service (QES, QSeal, QWAC, QTimestamp, etc.) are defined in Appendix A: Certificate Profiles and conform to RFC 5280 and ETSI EN 319 411-2 (as applicable).
-   The applicable Certificate Policy (CP) and Certification Practice Statement (CPS) are published at [REPOSITORY_URL] and referenced herein: CP §4 (Certificate Profiles), CPS §7 (Certificate, CRL, and OCSP Profiles).
-   Profiles are harmonized with Section 7 (Status Services) and Section 11 (Records) to ensure consistent validity periods and status freshness across OCSP/CRL publishing.
...
<!-- **Guidance:** This section covers the formal process by which a subscriber agrees to the terms of their certificate. It also outlines their responsibilities, such as protecting their private key. -->
## 5. CERTIFICATE ACCEPTANCE AND USAGE
...
<!-- **Guidance:** A trustworthy TSP must have a reliable and always-available revocation process. This section must detail *exactly* how a user can request revocation (especially in an emergency) and the steps you take to authenticate that request. -->
## 6. REVOCATION PROCEDURES
...
<!-- **Guidance:** Describe the technical details of your public revocation status services. Auditors will test the availability, performance, and accuracy of these services. -->
## 7. STATUS CHECKING SERVICES (OCSP & CRL)

### 7.1. OCSP Service
-   **Availability:** The OCSP service is provided with a **99.95%** annual uptime SLA (aligned with Revocation SOP §5.2).
...
-   **Response Time:** The service is designed to provide responses in under **150ms** (95th percentile), aligned with Revocation SOP §5.2.
-   **Caching/Validity:** OCSP responses include `thisUpdate`/`nextUpdate` with `nextUpdate` set to **4 hours**; relying parties must not cache beyond `nextUpdate`.
...
### 7.2. CRL Service
-   **Full CRL:** Published every **12 hours**. The `nextUpdate` field is set to **13 hours** to provide an overlap period (aligned with Revocation SOP §6.1).
-   **Delta CRL:** Published every **1 hour**. The `nextUpdate` field is set to **70 minutes**.
...
<!-- **Guidance:** This section is the detailed implementation of the security promises made in the TSP Policy. Provide specifics about your cryptographic, physical, and logical security controls. -->
## 8. SECURITY MEASURES AND CONTROLS
...
### 8.4. QSCD Remote Signing Mapping (CEN EN 419 241-1/-2)

This section maps our remote signing implementation to CEN EN 419 241-1/-2 and complements APPENDIX_A_QSCD_Remote_Signing.md:
- Architecture components: Signature Activation Module (SAM), Signature/Seal Application Module (SSA), and HSM boundary; secure channel and authenticated session between subscriber and SAM.
- Sole control: strong multi-factor authentication (e.g., PIN + OTP/FIDO2/biometric) to ensure the subscriber exclusively controls Signature Activation Data (SAD) and authorizes each signature.
- Signature Activation Protocol: per EN 419 241-2, binding of the document hash set (ToBeSigned) to the authenticated session and subscriber identity; anti-replay protections (nonces, timestamps).
- SAD protection: generation, storage, and use restricted within SAM/SSA boundary; no export; lifecycle managed with auditable events.
- Transaction binding: each remote signing request includes signed attributes linking the specific TBS data, time, subscriber identifier, and policy OID; server-side checks enforce policy and rate limits.
- Audit & evidence: comprehensive event logging (authentication, authorization, signing request, HSM operation result, errors) with integrity protection and time synchronization; logs retained per §11.
- Key lifecycle: key generation, storage, backup, and destruction performed in certified HSMs; M-of-N controls for administrative operations; periodic control reviews.
- Incident & DR: failure modes covered by IRP/DRP; subscriber notification on suspicious activity; rapid revocation path integrated with §6 and §7.

Conformance references:
- EN 419 241-1 (General System Security Requirements)
- EN 419 241-2 (Protection Profile for TSP Remote QSCD)
- APPENDIX_A_QSCD_Remote_Signing.md (detailed implementation and test evidence)
...
<!-- **Guidance:** eIDAS requires that personnel in "trusted roles" (e.g., RA operators, system administrators, crypto officers) be carefully vetted and trained. Describe your procedures for background checks, training, and ensuring segregation of duties. -->
## 9. PERSONNEL REQUIREMENTS
...
<!-- **Guidance:** You must be able to prove who did what, and when. This section details what events are logged, how logs are protected from tampering, and how long they are kept. This is a critical evidence source for auditors. -->
## 10. AUDIT LOGGING AND MONITORING
...
<!-- **Guidance:** Define your policy for long-term data storage. This must comply with eIDAS, EETT, and national commercial/tax laws. Explain how you balance the need to retain evidence with the GDPR "right to be forgotten." -->
## 11. RECORDS ARCHIVAL AND RETENTION
...
<!-- **Guidance:** This section describes your plans for handling service disruptions, from minor outages to major disasters. It should reference your formal BCP and DRP documents and state your target recovery times (RTO) and data loss tolerances (RPO). -->
## 12. DISASTER RECOVERY AND BUSINESS CONTINUITY
...
<!-- **Guidance:** A summary of the legal framework and audit commitments. -->
## 15. COMPLIANCE AND LEGAL PROVISIONS
...
<!-- **Guidance:** This section defines the governance for this document itself—who owns it, how it's updated, and how users are notified of changes. -->
## 17. PUBLICATION AND AMENDMENTS
